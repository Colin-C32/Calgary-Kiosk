...
const PlanATrip = ({ setPage }) => {
  ...
  const updateSearchResults = (query) => {
    const service = new window.google.maps.places.PlacesService(map_ref.current);
  ...
  const nextPage = () => {
    const new_page = current_page + 1 > page_count ? current_page : current_page + 1
    ...
    <div className="plan-a-trip-page">
      ...
      <div className="search-bar">
        ...
        {isDropdownVisible && (
          <div className="search-bar-results">
            {search_results.length > 0 ? (search_results.slice(0, 5).map((place, index) => (
              <button key={index} onClick={() => selectLocation(place.geometry.location)}>
      ...
      <div className="map-directions-actions">
        <div className="map-directions-mode">
          <button className={travel_mode === "WALKING" ? 'active-mode' : ''} onClick={() => setTravelMode("WALKING")}>
            <DirectionsRunIcon></DirectionsRunIcon>
          </button>
          <button className={travel_mode === "TRANSIT" ? 'active-mode' : ''}  onClick={() => setTravelMode("TRANSIT")}>
            <DirectionsRailwayIcon></DirectionsRailwayIcon>
          </button>
          <button className={travel_mode === "DRIVING" ? 'active-mode' : ''}  onClick={() => setTravelMode("DRIVING")}>
            <DirectionsCarIcon></DirectionsCarIcon>
          </button>
        </div>
        {location && (
          <button className="map-directions-close" onClick={() => setLocation(null)}>
          <CloseOutlinedIcon/>
          </button>
        )}
      </div>
        ...
        <div className="search-results">
          ...
          <div className='search-results-body'>
            {currentItems.map((place, index) => (
              <button className="search-result" key={index} onClick={() => selectLocation(place.geometry.location)}>
                <span className='search-name'>{place.name}</span>
                <span className='search-distance'>{(place.distance).toFixed(2)} km</span>
              </button>
            ))}
    ...
    <div style={indentStyle}>
      {transit_details ? (
        <div className='step-transit'>
          <span className='step-transit-header'>Ride the <b>{transit_details.line.vehicle.type}</b> towards <b>{transit_details.headsign}</b></span>
          <span className='step-transit-fromto'>From: {transit_details.departure_stop.name} ({transit_details.departure_time.text})</span>
          <span className='step-transit-fromto'>To: {transit_details.arrival_stop.name} ({transit_details.arrival_time.text})</span>
          {step.steps && step.steps.map((nestedStep, index) => (
            <StepComponent key={`${level+1}_${index}`} step={nestedStep} level={level + 1} />
          ))}
        </div>
      ) : (
      ...
      {nested_steps && nested_steps.map((nested_step, index) => (
        <StepComponent key={`${level+1}_${index}`} step={nested_step} level={level + 1} />
      ))}
    </div>
  );
};
